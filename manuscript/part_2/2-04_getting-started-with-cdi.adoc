...
This is exactly for this kind of situation that the `@Alternative` concept has been created in CDI.

==== Creating an `@Alternative` bean

From a technical point of view, creating a specialized bean is really simple. We just need to add the `@Alternative` annotation where the bean is produced and we are done.
...
Instead of doing it that way, we can also add the declaration of alternatives in the `beans.xml` file. Using this way, we do not need to recompile our application, we only need to reboot it.  
