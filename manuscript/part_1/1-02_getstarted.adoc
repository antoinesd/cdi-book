...
include::1-02_schema_microblogging.adoc[]
...
As the world of Java IDE (Integrated Development Environment) is moving rather fast (roughly a new major version each year), we decided to focus the example on the book on command line tools.
...
As the command line tools we use in the book are perfectly integrated with the major Java IDE, there won't be any issue to use these tools and your IDE.
...
As we already said, the minimum tool requirement to follow the book example will be Maven.
...
Go to http://www.oracle.com/technetwork/java/javase/downloads, click on the Java Download button.
...
Should you encounter issues during the process, go back to the download page http://www.oracle.com/technetwork/java/javase/downloads and check the _Installation Instructions_ link.

===== Linux

While it's possible to install Oracle JDK under Linux (check the _Installation Instructions_ link on http://www.oracle.com/technetwork/java/javase/downloads if you want to do so), you might prefer install Open JDK packaged for your linux distribution.
...
the more straightforward way to install Maven is to download it from https://maven.apache.org/download.cgi and follow the install instruction on the page https://maven.apache.org/install.html
