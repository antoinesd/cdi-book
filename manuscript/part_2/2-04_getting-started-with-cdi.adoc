...
It is legal for a qualifier to define annotation members. In that case, CDI will compare the two annotations along with the values of their members. Those values are compared with their respective `equals()` method, as we expected.

A qualifier may also define members that are not taken into account in the comparison. In this case, this member should be annotated with `@Nonbinding`.

In the case an annotation member type is an array, then it should be annotated with `@Nonbinding`. If not, then non predictable or implementation dependent result will occur.
...
----
<1> We do not need to tell that his member is the `value()` attribute, due to the way annotations work in Java
...
Any number of qualifiers can be added on a CDI injection point. CDI will look for a bean that matches all the present qualifiers. In no bean, or more than one bean matches the definition, then an error will be raised.
...
This qualifier is present by default on all producers, and all injection points. Thus the error message we saw when we created ambiguous injection points and producers.
